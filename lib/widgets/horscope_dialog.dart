  ...
  Future<void> fetchCities(String country) async {
    try {
      ...
      final response = await http.get(
        Uri.parse(
          'http://api.geonames.org/searchJSON?country=$countryCode&featureCode=PPL&maxRows=1000&username=ahamed',
        ),
      );
      ...
      if (response.statusCode == 200) {
        ...
        if (data['geonames'] != null) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('No cities found for this country.')),
          );
        }
      } else {
 ...
 List<String> years = List.generate(
  ...
  Widget _buildFormContent() {
    ...
    child: GestureDetector(
      ...
      child: const AstrologerConsultationDialog(),
    ),
  );
    } else {
      return Form(
